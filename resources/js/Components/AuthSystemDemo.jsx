...
export default function AuthSystemDemo() {
  ...
  return (
    <div style={{ maxWidth: 400, margin: '2rem auto', padding: 20, border: '1px solid #ccc', borderRadius: 8 }}>
      ...
      {token && <div style={{ marginTop: 10 }}><b>Token:</b> <code>{token}</code></div>}
      <div style={{ marginTop: 20 }}>
        <button onClick={fetchUser} disabled={!token}>Fetch User Info</button>
        <button onClick={fetchPurchases} disabled={!token} style={{ marginLeft: 10 }}>Fetch Purchases</button>
      </div>
